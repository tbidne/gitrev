...
--
-- @since 2.0
module Development.GitRev.Utils.OsString
...
import Development.GitRev.Utils.Environment.OsString (LookupEnvError)
import Development.GitRev.Utils.Environment.OsString qualified as Env
import Development.GitRev.Utils.Git.OsString (GitError)
...
-- >>> import Development.GitRev.Utils.Git.OsString (GitError (..), gitDirtyQ, gitHashQ)
-- >>> import Development.GitRev.Utils.Environment.OsString (LookupEnvError (..))
...
projectError = projectErrorMap (Internal.encodeLenient . displayException)
...
projectErrorMap ::
  ...
  f (Either e a) ->
  f a
projectErrorMap onErr = projectLeft (error . Internal.decodeLenient . onErr)
