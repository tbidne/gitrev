...
--
-- @since 2.0
module Development.GitRev.Utils.Git.Internal
  ( GitError (..),
    IndexUsed (..),
    GitProcessArgs (..),
    runGitPostprocess,
    ...
    then do
      ...
      when hdExists $ do
        ...
        T.splitAt 5 <$> runIO (readFileUtf8 hd) >>= \case
          -- pointer to ref
          ("ref: ", relRef) -> do
            relRefOs <- Internal.encodeThrowM $ T.unpack relRef
            let ref = pwd </> tillNewLineOsPath relRefOs
            refExists <- runIO $ doesFileExist ref
            refFp <- Internal.decodeThrowM ref
      ...
      when (indexExists && useIdx == IdxUsed) $ do
        indexFp <- Internal.decodeThrowM index
      ...
      when packedExists $ do
        packedRefsFp <- Internal.decodeThrowM packedRefs
      ...
      if isFile
        then do
          T.splitAt 8 <$> readFileUtf8 dotGit >>= \case
            ("gitdir: ", relDir) -> do
              relDirOs <- Internal.encodeThrowM $ T.unpack relDir
