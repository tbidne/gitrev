...
import Development.GitRev.Internal.Environment.OsString (EnvError)
...
--       q = pure (Left $ MkGitError [osstr|not found|])
...
--       q = pure (Left $ MkEnvError [osstr|VAR|] Nothing [osstr|VAR does not exist|])
